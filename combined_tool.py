...
WITH GOOGLE SHEETS API RATE LIMITING FOR STREAMLIT CLOUD
"""
...
class GoogleSheetsRateLimiter:
    ...
    def __init__(self, min_delay=1.5, max_requests_per_100s=80):
        """
        Initialize rate limiter using Streamlit session state
        
        Args:
            min_delay: Minimum delay between API calls in seconds
            max_requests_per_100s: Maximum requests per 100 seconds (set to 80, well below Google's 100 limit)
        """
    ...
    def wait_if_needed(self, show_status=False):
        ...
        # Check rate limit (requests per 100 seconds)
        if len(st.session_state.sheets_api_timestamps) >= self.max_requests_per_100s:
            ...
            wait_for_window = 100 - (current_time - oldest_request) + 1  # 1s buffer
...
class GoogleSheetsExporter:
    ...
    def __init__(self, credentials_dict: Dict):
        ...
        # Use longer delays for Streamlit Cloud environment
        self.rate_limiter = GoogleSheetsRateLimiter(min_delay=1.5)
        
        # Track API calls in session state for visibility
...
def main():
    ...
    with st.sidebar:
        ...
        # Add rate limit control for Video Rater
        if mode == "Video Rater":
            st.subheader("Rate Limiting Settings")
            processing_delay = st.slider(
                "Additional delay between videos (seconds)",
                min_value=0.0,
                max_value=5.0,
                value=1.0,
                step=0.5,
                help="Extra delay on top of automatic rate limiting"
            )
    ...
    if mode == "Data Collector":
        ...
        if not youtube_api_key or not sheets_creds or not spreadsheet_id:
            ...
            # Rating control with processing delay
            processing_delay = st.sidebar.slider(
                "Delay between videos (seconds)",
                min_value=0.5,
                max_value=5.0,
                value=2.0,
                step=0.5,
                help="Delay between processing each video to avoid API limits"
            ) if 'processing_delay' not in locals() else processing_delay
                        ...
                        with st.spinner("Analyzing video..."):
                            ...
                            if video_id:
                                try:
                                    ...
                                    # Process the video automatically with rate limiting
                                    ...
                                    # Apply user-configured delay between videos
